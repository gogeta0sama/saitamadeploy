...
from SaitamaRobot import CMD_LIST, LOAD_PLUG, tbot
import glob
import sys
from SaitamaRobot import ubot
from pymongo import MongoClient
from SaitamaRobot import MONGO_DB_URI
from SaitamaRobot.modules.sql.checkuser_sql import add_usersid_in_db, already_added, get_all_users
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"SatamaRobot/modules/{shortname}.py")
        name = "Saitama.modules.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        mod.register = register
        mod.SaitamaRobot = Saitamarobot
        mod.tbot = tbot
